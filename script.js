  ...
  function deleteTak(e) {
    ...
    var takObjectOfTakDeleted = takList.filter((tak)=>{return tak.id==e.target.dataset.id})[0];
    sessionStorage.setItem("tak-deleted",JSON.stringify(takObjectOfTakDeleted));
  ...
  function undoDelete(){
    var takObject = JSON.parse(sessionStorage.getItem("tak-deleted"));
    $alertMessage.innerText = 'Deshaciendo';
    messageToggle(false);
    sessionStorage.setItem('saved-id', takObject.id);
    insertRows(END_POINT, 'lista', [takObject], null, (list) => {
      renderList(list);
      var id = sessionStorage.getItem('saved-id');
      var saved = document.getElementById(id);
      saved.scrollIntoView();
